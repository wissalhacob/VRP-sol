...
df_village = pd.read_excel("./VRP/Coordonnées_cleaned.xlsx", index_col=None)
df_depot = pd.read_excel("./VRP/Dépot.xlsx")
df_besoin = pd.read_excel("./VRP/Population_cleaned.xlsx")
...
# Utilisation des chemins optimisés dans l'algorithme 2-opt
for path in optimized_paths:
    i = 0  # Indice de départ
    while i < len(path) - 1:
        for j in range(i + 2, len(path)):
            for k in range(len(df_depot)):  # Parcourir tous les dépôts
                depot_coords = (df_depot.loc[k, 'y'], df_depot.loc[k, 'x'])  # Coordonnées du dépôt actuel
                new_path = two_opt(path, i, j)  # Appel correct de two_opt avec les indices i et j
                if calculate_distance(new_path, depot_coords) < calculate_distance(path, depot_coords):
                    path = new_path
        i += 1

# Génération du fichier Excel
df_paths = pd.DataFrame(columns=['Jour', 'Dépôt', 'Véhicule', 'Chemin'])
my_map = folium.Map(location=[31.7917, -7.0926], zoom_start=7, tiles='CartoDB PositronNoLabels')
...
for day in range(1, nb_days + 1):
    buttons_html += f"""
    <button onclick="toggle_day({day});">Jour {day}</button>
"""
...
# Ajout de la fonction JavaScript toggle_day au code HTML de la carte
toggle_script = """
<script>
function toggle_day(day) {
    var elements = document.querySelectorAll('[data-day]');
    console.log("Boutton selectione le jour : " + day);
...
my_map.get_root().html.add_child(Element(toggle_script))
...
path_to_output = "./VRP/Chemins.xlsx"
